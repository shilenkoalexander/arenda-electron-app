<template>
    <v-card class="elevation-1 fill-height">
        <v-container fluid class="px-5">
            ...
            <v-row>
                <v-col cols="3">
                    <Label label="Дата начала расчета" :value="formatToFriendlyByDate(mainInfo.calculationStartDate)"/>
                </v-col>
                <v-col cols="3">
                    <Label label="Дата заключения договора"
                           :value="formatToFriendlyByDate(mainInfo.calculationStartDate)"/>
                </v-col>
                <v-col cols="3">
                    <Label label="Дата подписания договора" :value="
                           formatToFriendlyByDate(mainInfo.calculationStartDate)"/>
                </v-col>
            </v-row>
        </v-container>
    </v-card>
...
<script lang="ts">
    ...
    import { ContractPageMainInfo } from '@/model/types/contract-types';
    ...
    export default class ContractInfo extends Vue {
        ...
        formatToFriendlyByDate = formatToFriendlyByDate;
    }
</script>
